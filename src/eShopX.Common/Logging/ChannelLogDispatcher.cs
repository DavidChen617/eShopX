...
public sealed class ChannelLogDispatcher : ILogDispatcher, IDisposable
{
    ...
    public ChannelLogDispatcher(IOptionsMonitor<CustomLoggerOptions> options)
    {
        _options = options;
        ...
        var channelOptions = new BoundedChannelOptions(capacity)
        {
            SingleReader = true,
            SingleWriter = false,
            FullMode = _options.CurrentValue.ChannelFullMode
        };
    ...
    private void Write(LogEntry entry)
    {
        ...
        {
            if (sink is ConsoleSink consoleSink && entry.Color.HasValue)
            {
                consoleSink.Emit(message, entry.Color.Value);
            }
            else
            {
                sink.Emit(message);
