    ...
    ICacheService cacheService) : IRequestHandler<GetProductsQuery, GetProductResponse>
{
    public async Task<GetProductResponse> Handle(GetProductsQuery query, CancellationToken cancellationToken = default)
    {
        ...
        var normalizedQuery = query with { Page = page, PageSize = size };

        if (!string.IsNullOrWhiteSpace(normalizedQuery.Keyword))
        {
            try
            {
                return await productSearchService.SearchAsync(normalizedQuery, cancellationToken);
            }
            catch (ExternalServiceException)
            {
                // Elasticsearch search failed. Fallback to DB query.
            }
        }

        return await QueryAsync(normalizedQuery, page, size, cancellationToken);
    }
    
    private async Task<GetProductResponse> QueryAsync(
          ...
          return (await cacheService.GetOrSetAsync(
              ...
              async ct =>
              {
                  ...
                  var primaryImages = await productImageRepository.ListAsync(
                      x => productIds.Contains(x.ProductId) && x.IsPrimary, ct);
