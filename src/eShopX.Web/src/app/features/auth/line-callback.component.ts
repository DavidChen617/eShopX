  ...
  template: `
    <section class="flex min-h-[60vh] items-center justify-center px-4 py-12">
      <div class="text-center">
        <div class="text-lg font-semibold text-slate-900">LINE 登入中...</div>
        @if (errorMessage()) {
...
export class LineCallbackComponent {
  ...
  constructor() {
    ...
    if (!code || !state) {
      this.errorMessage.set('缺少 LINE 授權資訊，請重新登入。');
      return;
    }

    this.authService.exchangeLineCode(code, state).subscribe({
      next: () => {
        this.authService.clearLineState();
        void this.router.navigateByUrl('/');
      },
      error: (error: unknown) => {
        this.authService.clearLineState();
        this.errorMessage.set(this.resolveErrorMessage(error, 'LINE 登入失敗，請稍後再試。'));
      },
    });
  }
